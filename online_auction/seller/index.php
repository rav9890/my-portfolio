...
if(isset($_POST['login']))
{
  ...
  $select = "SELECT * FROM tbl_sellers WHERE username = '".$user."' AND password = '".$pass."' ";
  $selected = mysqli_query($con,$select);
  $row = mysqli_num_rows($selected);
  
  while($show = mysqli_fetch_assoc($selected))
  {
    $_SESSION['seller_id']=$show['seller_id'];
    $_SESSION['name']=$show['name'];
    $_SESSION['company']=$show['company'];
  }
...
<body>
		...
		<div class="row">
			<div class="col-sm-4">
				...
				<form method="post">
					...
					<h3 style="text-align:center;color:white;">SELLER LOGIN</h3>
					<br>
 ...
 <br>
  ...
  <center><p style="color:black;font-size:18px;">Create account <a href="seller_signup"><i>Sign Up</i></a> here</p></center>
